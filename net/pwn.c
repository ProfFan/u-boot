...
static ushort pwn_seq_number;

/* The ip address to pwn */
struct in_addr net_pwn_ip;

static void set_pwn_header(uchar *pkt, struct in_addr dest)
{
	/*
	...
	icmp->checksum = 0;
	icmp->un.echo.id = 0;
	icmp->un.echo.sequence = htons(pwn_seq_number++);
	icmp->checksum = compute_ip_checksum(icmp, ICMP_HDR_SIZE);
}

static int pwn_send(void)
{
	uchar *pkt;
	int eth_hdr_size;
...
static void pwn_timeout_handler(void)
{
	eth_halt();
...
void pwn_start(void)
{
	...
	net_set_timeout_handler(10000UL, pwn_timeout_handler);
...
void pwn_receive(struct ethernet_hdr *et, struct ip_udp_hdr *ip, int len)
{
	...
	case ICMP_ECHO_REPLY:
		...
		if (src_ip.s_addr == net_pwn_ip.s_addr)
